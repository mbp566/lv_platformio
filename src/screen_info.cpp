...
InfoScreen::InfoScreen(Client *client) :
  Screen(client)
{
...
{
  m_client->updateInfo();
  lv_label_set_text_fmt(m_lines[0].value, "%s", m_client->info()->version);
  lv_label_set_text_fmt(m_lines[1].value, "%s", m_client->info()->waterHeaterIP);
  lv_label_set_text_fmt(m_lines[2].value, "%s", m_client->info()->remoteIP);
