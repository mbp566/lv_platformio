...
static uint8_t buffer[SCREEN_WIDTH * (SCREEN_HEIGHT - LINE_HEIGHT - LINE_HEIGHT) * 4];

HistoryScreen::HistoryScreen(Client *client) : Screen(client)
{
...
void HistoryScreen::update()
{
  ...
  for (int i = 0; i < m_client->nextHistoryIndex(); i++)
  {
    ...
    pointsLow[i] = { x, CHART_TOP + CHART_HEIGHT * (100 - m_client->history()[i].tempLow) / 100 };
    pointsHigh[i] = {x, CHART_TOP + CHART_HEIGHT * (100 - m_client->history()[i].tempHigh) / 100};
    pointsPanel[i] = {x, CHART_TOP + CHART_HEIGHT * (100 - m_client->history()[i].tempPanel) / 100};
  }
  ...
  drawCurve(&layer, pointsLow, m_client->nextHistoryIndex(), LV_COLOR_MAKE(0xE9, 0xC4, 0x6A), "Tank", 0);
  drawCurve(&layer, pointsHigh, m_client->nextHistoryIndex(), LV_COLOR_MAKE(0xE7, 0x6F, 0x51), "Water", 1);
  drawCurve(&layer, pointsPanel, m_client->nextHistoryIndex(), LV_COLOR_MAKE(0x36, 0xBA, 0x98), "Panel", 2);
...
void HistoryScreen::drawHeat(lv_layer_t *layer)
{
  int start = 0;
  int i = 1;
  uint8_t heatStatus = m_client->history()[0].heat;
  while (i < 144)
  {
    while ((i < 144) && (m_client->history()[i].heat == heatStatus)) {
      i++;
    }
    ...
    start = i;
    heatStatus = m_client->history()[i].heat;
  }
}
...
void HistoryScreen::drawPump(lv_layer_t *layer)
{
  int start = 0;
  int i = 1;
  bool pump = m_client->history()[0].pump;
  while (i < 144)
  {
    while ((i < 144) && (m_client->history()[i].pump == pump)) {
      i++;
    }
    if (pump) {
      lv_draw_rect_dsc_t rect_dsc;
